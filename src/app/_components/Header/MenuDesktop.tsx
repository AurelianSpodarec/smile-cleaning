...
import { authLogout, getUserProfile } from "@/services/apis/requests/auth";
  ...
  return (
    <nav className="fixed left-0 right-0 px-16 py-4 w-full flex items-center justify-between">
      ...
      <div className="flex items-center align-center space-x-2">
        {!isLogged &&
          <>
            <Link href="/" className="border border-[#ae85b9] rounded-2xl px-4 py-2 font-sm text-white">Sign Up</Link>
            <form
              action={async () => {
                'use server'
            ...
            >
              <button className="bg-[#ae85b9] rounded-2xl inline-block px-4 py-2 font-sm">Login</button>
            </form>
          </>
        }

        <Link href="/schedule" className="hidden lg:flex items-center gap-1 nav-cta bg-[#a283ab] rounded-xl py-2 px-4 text-sm text-white border border-[gold] font-semibold">Schedule a cleaner</Link>

        {isLogged &&
          <UserAvatar menu={userMenu} email={session?.user?.fullName} avatar="https://media.licdn.com/dms/image/v2/D4D03AQE8vC6NMPCXyA/profile-displayphoto-shrink_200_200/profile-displayphoto-shrink_200_200/0/1710696258127?e=2147483647&v=beta&t=wxyY2sBkviJSZMo5q9llLtYVi5VM9SaFVgE9A-5DMHA" />
        }
